...
namespace loess
{
    public class LoessInterpolator
    {
        ...
        /**
         * The bandwidth parameter: when computing the loess fit at a particular point, this fraction of source points closest
         ...
         */
        public static void updateBandwidthInterval(double[] xval, int i, int[] bandwidthInterval)
        {
            int left = bandwidthInterval[0];
            int right = bandwidthInterval[1];

            // The edges should be adjusted if the previous point to the left is closer to x than the current point to the right or
            // if the next point to the right is closer to x than the leftmost point of the current interval
        ...
        {
            double ntmp = Math.Abs(x);
            double tmp = 1 - ntmp * ntmp * ntmp;
            return tmp * tmp * tmp;
        }
         ...
         */
        public static void checkAllFiniteReal(double[] values, bool isAbscissae)
        {
         ...
         */
        public static void checkStrictlyIncreasing(double[] xval)
        {
    ...
    class Program
    {
        ...
        {
            LoessInterpolator Loess1 = new LoessInterpolator();
            Loess1.smooth(); 
        }
    }
}
